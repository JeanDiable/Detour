...
namespace DetourCore.Algorithms
{
    public class TightCoupler
    {
        ...
        public static double[] CordinateDescend(double[] initial, double[] ordinals, double[] targets, 
        ...
        public static void Helper(CartLocation[] hLs, CartLocation[] nhLs)
        {
            ...
            if (ixs == null)
            {
                ...
                xs = ixs = WeightedRegression.Weighted(samples, hLs.Select(p => (double) p.x).ToArray(), ws);
                ys = iys = WeightedRegression.Weighted(samples, hLs.Select(p => (double) p.y).ToArray(), ws);
                ...
                ths = iths = WeightedRegression.Weighted(samples, oths, ws);
            }
            else
            {

                double[] sxs = new double[3]
                    {ixs[0] + ixs[1] * tDiff + ixs[2] * tDiff * tDiff, ixs[1] + 2 * ixs[2] * tDiff, ixs[2]};
                double[] sys = new double[3]
                    {iys[0] + iys[1] * tDiff + iys[2] * tDiff * tDiff, iys[1] + 2 * iys[2] * tDiff, iys[2]};
                double[] sths = new double[3]
                    {iths[0] + iths[1] * tDiff + iths[2] * tDiff * tDiff, iths[1] + 2 * iths[2] * tDiff, iths[2]};

                for (int k = 0; k < 5; ++k)
                {
                    xs = CordinateDescend(sxs, ordinal, hLs.Select(p => (double) p.x).ToArray(),
                        0.001 * (mTime - vTime) * (mTime - vTime),
                        0.001 * (mTime - vTime),
                        0.001);
                    ys = CordinateDescend(sys, ordinal, hLs.Select(p => (double) p.y).ToArray(),
                        0.001 * (mTime - vTime) * (mTime - vTime),
                        0.001 * (mTime - vTime),
                        0.001);
                    var oths = hLs.Select(p => (double) p.th).ToArray();
                    ...
                    ths = CordinateDescend(sths, ordinal, oths,
                        0.00031 * (mTime - vTime) * (mTime - vTime),
                        0.00031 * (mTime - vTime),
                        0.00031);
