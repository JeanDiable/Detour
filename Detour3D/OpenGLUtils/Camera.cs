...
namespace LidarController.OpenGLUtils
{
    class Camera
    {
        ...
        {
            // var factor = 0.1f + (float)Math.Pow(_distance - _minDist, 0.5);
        ...
        {
            _distance = MathHelper.Clamp(_distance * (1 + delta), _minDist, float.MaxValue);
            UpdateInternalParams();
        }

        public Camera(Vector3 stare, float dist, float width, float height, float minDist)
        {
            Reset(stare, dist);
        ...
        public Matrix4 GetProjectionMatrix()
        {
            return _projectionMode == 0 ?
                Matrix4.CreatePerspectiveFieldOfView(_fov, _aspectRatio, 0.01f, 10000f) : 
                Matrix4.CreateOrthographic(_width * _distance / 350, _height * _distance / 350, 0.01f, 10000f);
        }
