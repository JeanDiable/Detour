...
__kernel void rect_decoding(global const unsigned char* rectified, global int* len, int iw, global int* result, global int* ordering) {
	...
	for (int i = 0; i < 36 / reduce_sz + (36 % reduce_sz > 0); ++i) {
		...
		if (me < 36) {
			...
			SWAP(a, 3, 5);
			SWAP(a, 3, 4); 
			int lv = a[1] - a[0], now, split=0;
			now = a[2] - a[1]; if (now > lv) { lv = now;split = 1; }
			now = a[3] - a[2]; if (now > lv) { lv = now;split = 2; }
			now = a[4] - a[3]; if (now > lv) { lv = now;split = 3; }
			now = a[5] - a[4]; if (now > lv) { lv = now;split = 4; } 
			now = a[6] - a[5]; if (now > lv) { lv = now;split = 5; }
			now = a[7] - a[6]; if (now > lv) { lv = now;split = 6; }
			if (lv > (a[7] - a[0]) * 0.2f) { // bw distinguishable.
				if (abs_diff(a[split+1], cval[mid]) < abs_diff(a[split], cval[mid]))
					c[me] = 0;  
				else
					c[me] = 1;
			}
