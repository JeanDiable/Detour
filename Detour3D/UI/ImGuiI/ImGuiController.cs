...
namespace Dear_ImGui_Sample
{
    ...
    public class ImGuiController : IDisposable
    {
        ...
        public ImGuiController(int width, int height)
        {
            ...
            IntPtr context2 = ImPlot.CreateContext();
            ImPlot.SetCurrentContext(context2);
            Console.WriteLine("Set current context done");
...
void main()
{
    outputColor = color * texture(in_fontTexture, texCoord);
        ...
        public void UpdateWinForm(Form wnd)
        {
            if (_frameBegun)
            {
                ImGui.Render();
            ...
            var interval = ((double) (sw.ElapsedTicks - last)) / Stopwatch.Frequency;
            last = sw.ElapsedTicks;
            SetPerFrameImGuiData((float) interval);
            UpdateImGuiInputWinForm(wnd);
        ...
        public MouseState lastMouseState = Mouse.GetCursorState();
        
        private void UpdateImGuiInputWinForm(Form wnd)
        {
            ImGuiIOPtr io = ImGui.GetIO();

            // MouseState MouseState = Mouse.GetCursorState();
            MouseState ms = lastMouseState;
            KeyboardState KeyboardState = Keyboard.GetState();

            io.MouseDown[0] = ms[MouseButton.Left];
            io.MouseDown[1] = ms[MouseButton.Right];
            io.MouseDown[2] = ms[MouseButton.Middle];
            var pt = wnd.PointToClient(new Point(ms.X, ms.Y));
            io.MousePos = new System.Numerics.Vector2(pt.X, pt.Y);
