        ...
        public ICPResult ICPframe2frame(Queryer3D queryer, Lidar3D.Lidar3DFrame compared, QT_Transform initQt)
        {
            ...
            void maximize(bool firstIter)
            {
                ...
                void maximizeAngle()
                {
                    ...
                    //     cvec += txyzs[i]*w;
                    //     ovec += occurances[i].c*w;
                    //     sum1 += w;
                    // }
                    // cvec /= sum1;
                    // ovec /= sum1;
                    // Console.WriteLine($"vecC={vecC.X},{vecC.Y},{vecC.Z} vecD={vecD.X},{vecD.Y},{vecD.Z}");
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        ...
                        var vecO = (occurances[i].c - cvec) * w;
                        var vecA = (txyzs[i] - ovec) * w;
                    ...
                    //     var w = 1 - Vector3.Dot(queryer.planes[occurances[i].idx].lmn, axis);
                    //     sumpw += w;
                    //     pcO += occurances[i].c * w;
                    //     pcA += txyzs[i] * w;
                    // }
                    //
                    // pcO /= sumpw;
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        ...
                        var vecO = (occurances[i].c - pcO)*w;
                        var vecA = (txyzs[i] - pcA)*w;
                ...
                void update()
                {
                    ...
                    Console.WriteLine($"accum axis={aaxis.X}, {aaxis.Y}, {aaxis.Z}, th={aTh}");
                    
                    for (int i = 0; i < compared.reducedXYZ.Length; i++)
                    {
                        if (occurances[i].idx == -1 || !occurances[i].plane) continue;
                        var vec = qt.Transform(compared.reducedXYZ[i], compared.reduceLerpVal[i]);
                        txyzs[i] = vec;
                        occurances[i].c = Queryer3D.ProjectPoint2Plane(vec, queryer.planes[occurances[i].idx]).vec3;
                    }
                    Console.WriteLine(
                        $"T={qt.T.X}, {qt.T.Y}, {qt.T.Z}");
                }
                ...
                void maximizeTranslation()
                {
                    ...
                    qt.T += dvec;
                    // Console.WriteLine(
                    //     $"1>T={qt.T2.X}, {qt.T2.Y}, {qt.T2.Z}, R={qt.Q2.X},{qt.Q2.Y},{qt.Q2.Z},{qt.Q2.W}");
                ...
                for (int j = 0; j < 5; ++j)
                {
                    ...
                    update();
                    Console.WriteLine($"update={(double)(G.watch.ElapsedTicks - tic) / Stopwatch.Frequency * 1000}ms");

                    tic = G.watch.ElapsedTicks;
                    maximizeTranslation();
                    Console.WriteLine($"trans={(double)(G.watch.ElapsedTicks - tic) / Stopwatch.Frequency * 1000}ms");
