    ...
    public partial class Lidar3DOdometry : Odometry
    {
        ...
        public void loopPreprocess()
        {
            ...
            while (true)
            {
                ...
                var queryer = new RefinedPlanesQueryer3D(frame2process, lstat);
        ...
        [MethodMember(name = "DrawLocalMap", desc = "Draw local map and normal")]
        public void DrawLocalMap()
        {
            drawlocalmap = true;
        }

        [MethodMember(name = "Optimize", desc = "Optimize local RPA")]
        public void RPA()
        {
            Task.Factory.StartNew(() =>
            {
                Lidar3DKeyframe.GridContent[] grid;
                lock (optimizingKeyframe)
                    grid = optimizingKeyframe.grid.ToArray().Select(p => p.Value).ToArray();
                optimizingKeyframe.queryer.Optimize(grid, l3d);
            });
        }

        private bool drawlocalmap = false;
        [StatusMember(name = "local map planes count")] public int localMapNPlanes = 0;
        [StatusMember(name = "local map ICP + optimize time")] public double localICPTime = 0;
        [StatusMember(name = "merge scan + optimize time")] public double mergeTime = 0;
        // [StatusMember(name = "optimize time")] public double O_time = 0;
        public void loopRefine()
        {
            ...
            while (true)
            {
                ...
                void drawQueryer()
                {
                    ...
                    {
                        foreach (var p in g.scan.corrected)
                        {
                            var v3 = g.qt.Transform(p);
                    ...
                    foreach (var g in grid)
                    {
                        var len = g.scan.corrected.Length;
                        for (var i = 0; i < len; ++i)
                        {
                ...
                // add missing planes
                var tic = G.watch.ElapsedTicks;
                var notAppliedScans = grid.Where(p => !p.applied).ToArray();
                if (notAppliedScans.Length > 0)
                {
                    // todo: apply planes
                    foreach (var scan in notAppliedScans)
                    {
                        if (scan.scan.query != null)
                        {
                            scan.applied = true;
                            optimizingKeyframe.queryer.MergeScan(scan.scan, scan.qt); //, grid);
                        }
                    }
                    //drawQueryer();
                    localMapNPlanes = optimizingKeyframe.queryer.planes.Length;
                    mergeTime = Toc(tic);
                    // drawQueryer();
                }
                ...
                //tic = G.watch.ElapsedTicks;
                //var pickId = G.rnd.Next() % (grid.Length - 1) + 1;
                //var pickedScan = grid[pickId];
                //var rr = ICP_3D(optimizingKeyframe.queryer, pickedScan.scan.correctedReduced, pickedScan.qt);
                //pickedScan.qt = rr.qt;
                //// sector: recompute planes.
                //optimizingKeyframe.queryer.Recalculate(grid);
                //
                //localICPTime = Toc(tic);
                // drawQueryer();
                Thread.Sleep(100);
        ...
        [StatusMember(name = "local map reg time")] public double S2M_time = 0;
        public void loopMap()
        {
            ...
            while (true)
            {
                ...
                var tic = G.watch.ElapsedTicks;
                var rr = ICP_3D(templateKeyframe.queryer, comparedScan.correctedReduced, qt,
                    iters: 15, iterI: 2);
                S2M_time = Toc(tic);
                ...
                else
                {
                    ...
                    rr.qt.computeMat();
                    stqt.computeMat();
                    var deltaD = rr.qt.Solve(stqt);
                    var delta = comparedScan.deltaInc.rdi * deltaD;
                    ...
                    // pdelta = comparedScan.deltaInc.rdi;
                    ...
                    // 经过验证，有时候可能planes一开始就是歪的，导致修正也没用，还是需要点云多了后再修
                    // rr = ICP_3D(templateKeyframe.queryer, comparedScan.correctedReduced, qt,
                    //     iters: 1, iterI: 1, finalModify: comparedScan);
        ...
        public void loopICP()
        {
            ...
            while (true)
            {
                ...
                // ref point must be valid points (d>10)
                RefinedPlanesQueryer3D queryer = null;
                lock (lastFrame)
                {
                ...
                // Vector3[] observedReduced = currentFrame.correctedReduced = CorrectionFine(currentFrame.reducedXYZ, currentFrame.reduceLerpVal, lastDeltaInc, rdi);
                // currentFrame.corrected = CorrectionFine(currentFrame.rawXYZ, currentFrame.lerpVal, lastDeltaInc, rdi);
                ...
                lock (QtQueue)
                {
                    ...
                    if (QtQueue.Count == 0)
                    {
                        QtQueueHeadCompareKeyframe = optimizingKeyframe = new Lidar3DKeyframe()
                        {
                            ...
                            pc = observedReduced,
                            queryer = new RefinedPlanesAggregationQueryer()
                        };
        ...
        public static Vector3[] CorrectionFine(Vector3[] v3, float[] lerp, QT_Transform lastDelta, QT_Transform delta)
        {
            //return v3;
        ...
        public ICP3DResult ICP_3D(Queryer3D queryer, Vector3[] comparedReducedXYZ, QT_Transform initQt, int iters = 13,
            int iterI = 1, Lidar3D.Lidar3DFrame finalModify=null)
        {
            bool debugCoor = true;
            ...
            void maximize()
            {
                ...
                void maximizeAngle(int iter)
                {
                    ...
                    var det2 = (float)(U2.Determinant() * VT2.Determinant());
            ...
            if (finalModify!=null)
            {
                correspond();
                int segments = 16;
                var avg = new Vector3[segments+2];
                var avgw = new float[segments+2];
                for (int i = 1; i <= segments; ++i) avgw[i] = 1;

                for (int i = 0; i < finalModify.correctedReduced.Length; i++)
                {
                    if (occurances[i].idx == -1) continue;
                    var diff = (occurances[i].proj - txyzs[i]);
                    if (diff.Length() > 70) continue;
                    var j = segments * finalModify.reduceIdx[i] / finalModify.rawXYZ.Length + 1;
                    var w = occurances[i].w;
                    if (diff.Length() > 30) w = w * LessMath.gaussmf(diff.Length(), 20, 30);
                    avgw[j] += w;
                    avg[j] += diff;
                }

                for (int i = 1; i <= segments; ++i) avg[i] = avg[i] / avgw[i];
                avg[0] = avg[1];
                avg[segments+1] = avg[segments];
                
                for (int i = 0; i < finalModify.corrected.Length; i++)
                {
                    var j = (double)segments * i / finalModify.rawXYZ.Length + 0.5;
                    var lo = (int) Math.Floor(j);
                    var hi = (int) Math.Ceiling(j);
                    var mod = avg[lo] * (float) (j - lo) + avg[hi] / avgw[hi];
                    // qt.T(f.c[i])+mod=qt.T(f.c[i]+what)
                    finalModify.corrected[i] = qt.ReverseTransform(qt.Transform(finalModify.corrected[i]) + mod);
                    // finalModify.corrected[i] += mod;
                    // finalModify.corrected[i]=Vector3.One;
                }
            }
