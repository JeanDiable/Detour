...
namespace Fake.UI.MessyEngine.MEObjects
{
    class MEGridObject : MEAbstractObject
    {
        ...
        public AerialCameraControl acc;

        public MEGridObject(float visibleAngle, float blurAngle, Camera cma, AerialCameraControl aerialCameraControl)
        {
            this.shaderType = MEShaderType.SpecificGrid;
            this.shader = new MEShader(this.shaderType);
            this.camera = cma;
            acc = aerialCameraControl;
            ...
            // grid texts
            _font = new QFont("Consolas", 11, new QuickFont.Configuration.QFontBuilderConfiguration(false));
        ...
        public override void UpdateMeshData(List<Vertex> verticesList = null, List<uint> indicesList = null)
        {
            ...
            // generate data
            _indicesCnt = 0;
            var grid0 = GenerateGrid(mainUnit * level, 1, false, acc._distance);
            var grid1 = GenerateGrid(mainUnit, _mainAlpha, true, acc._distance);
            var grid2 = GenerateGrid(minorUnit, _minorAlpha, false, acc._distance);
        ...
        private (List<Vertex>, List<uint>) GenerateGrid(float unit, float maxAlpha, bool isMain, float dist)
        {
            ...
            _blurRadius = MathHelper.Clamp((float)Math.Tan(_blurAngle) * dist, 1, dist * 5);
