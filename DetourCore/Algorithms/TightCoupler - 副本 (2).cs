            ...
            for (int iter = 0; iter < 1; ++iter)
            {
                ...
                for (var j = i + 1; j < kfdump.Length; j++)
                {
                        ...
                        !cli.source.noMove)
                    {
                        ...
                        offset.x += cTup.Item1*w;
                        offset.y += cTup.Item2*w;
                        offset.num += w;
                ...
                foreach (var pair in offsets)
                {
                    ...
                    if (double.IsNaN(offset.x) || double.IsNaN(offset.y) || double.IsNaN(offset.th) ||
                        Math.Abs(offset.x) > 10000 || Math.Abs(offset.y) > 10000)
                        throw new Exception("?");
                    c.x = (c.x * w + (float) (offset.x)) / (1 + w);
                    c.y = (c.y * w + (float) (offset.y))/(1 + w);
                    // Console.WriteLine($"th:{c.th} += {offset.th}");
                    c.th += (float) (offset.th) * 1 / (1 + w);
                }
