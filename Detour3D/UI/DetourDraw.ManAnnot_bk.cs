...
namespace Detour3D.UI
{
    partial class DetourDraw
    {
        ...
        private static void ManualAnnotation()
        {
            ...
            ImGui.SameLine();
            if (ImGui.Button(label: "下一帧 (N)")) NextManualFrame();
            ...
            if (ImGui.DragFloat3("旋转", ref vecR, dragSpeed)) DeformationCompensation();
            _vecTrans[0] = vecT.X;
            _vecTrans[1] = vecT.Y;
            _vecTrans[2] = vecT.Z;
            _vecRot[0] = vecR.X;
            ...
            void ImGuizmoManipulate()
            {
                ...
                ref _modelMatrix[0])) DeformationCompensation();
        ...
        private static unsafe List<LidarPoint3D> ReadPointsData(string lidarPath)
        {
            ...
            out3d.points = new LidarPoint3D[len];
            fixed (byte* src = buf)
            fixed (void* dst = out3d.points)
                CopyMemory(dst, src + 8, len * sizeof(LidarPoint3D));
        ...
        private static void VisualizePoints(List<LidarPoint3D> rawPoints, string lidarPath, int frameCnt, float[] modelM)
        {
            ...
            for (var i = 0; i < rawPoints.Count; ++i)
            {
                ...
                var fire = azimuth - startAngle;
                if (fire < 0) fire += 360;

                points.Add(new Point3D(x, y, z, 0, clockWise == 1 ? fire / 360f : 1 - fire / 360f, (int)rawPoints[i].intensity));
