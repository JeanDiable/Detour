    ...
    class GroundGrid : Geometry
    {
        ...
        { 
            ...
            Assembly.GetExecutingAssembly()
                .GetManifestResourceStream($@"Fake.UI.GLRes.consola.ttf").CopyTo(mem);
        ...
        private List<float> GenerateGrid(float unit, float maxAlpha, bool isMain)
        {
            ...
            _blurRadius = (float)Math.Tan(_blurAngle) * _cameraDistance;
            ...
            {
                if (Math.Abs(i - center.Z) > _blurRadius) continue;
                var theta = Math.Acos((i - center.Z) / _blurRadius);
                float xOffset;
                if (Math.Abs(theta - Math.PI / 2) < 0.001) xOffset = _blurRadius;
                else
                {
                    var blurLineTan = (float)Math.Tan(theta);
                    xOffset = blurLineTan * (i - center.Z);
                }
