...
__kernel void first_count(global int* label, global int* count1, int N) {
	...
	int lab = label[x];
	if (lab < 0) return;
	if (count1[lab] < component_sz_thres)
...
__kernel void reduce_count(global int* label, global int* count1, int N) {
	...
	int lbl = label[id];
	if (lbl < 0) return;
	if (count1[lbl] < component_sz_thres)
		label[id] = -1;
}
...
__kernel void relabel_points(global int* label, global unsigned int* len, global int* pos, int N) {
	...
	// local size should <512.
	local int my_psum[reduce_sz];
	// filter out valid points:
	for (int i = 0; i < group_sz / reduce_sz; ++i) {
		int x0 = group_st + x + (i * reduce_sz);
		bool is_point = (x0 < N) && (label[x0] == x0);
		my_psum[x] = is_point; barrier(CLK_LOCAL_MEM_FENCE);
		for (int j = 1; j <= reduce_sz; j += j) {
			if (x >= j) my_psum[x] += my_psum[x - j]; barrier(CLK_LOCAL_MEM_FENCE);
		}
		if (is_point) {
			int p = sz_boundaries + my_psum[x] - 1;
			if (p < group_comps)
				pos[out_st + p] = x0;
		}
		barrier(CLK_LOCAL_MEM_FENCE);
		if (x == reduce_sz - 1)
			sz_boundaries += my_psum[x];
	}
...
__kernel void merge_pos(global int* label, global int* len, global int* pos, int group_n) {
	...
	for (int i = 0; i < group_n / reduce_sz + (group_n % reduce_sz > 0); ++i) {
		...
		my_psum[x] = mylen; barrier(CLK_LOCAL_MEM_FENCE);
		for (int j = 1; j <= reduce_sz; j += j) {
			if (x >= j) my_psum[x] += my_psum[x - j]; barrier(CLK_LOCAL_MEM_FENCE);
		}

		for (int j = 0; j < mylen; ++j) {
			int nid = pstart + my_psum[x] - mylen + j;
			if (nid < max_lines)
...
// filter out lines with length <20px.
__kernel void reduce_line(global int* len, global float* lines, global float* rlines) {
	...
	for (int i = 0; i < len[0] / reduce_sz + (len[0] % reduce_sz > 0); ++i) {
		...
		bool is_point = x0 < len[0] && lines[x0 * 12 + 7] >= 256; //length^2

		my_psum[me] = is_point; barrier(CLK_LOCAL_MEM_FENCE);
		for (int j = 1; j <= reduce_sz; j += j) {
			if (me >= j) my_psum[me] += my_psum[me - j]; barrier(CLK_LOCAL_MEM_FENCE);
		}

		int pos = sz_lines + my_psum[me] - 1;
...
// assume maximum 256 lines. update: asumption fucked...
// todo: use bit operations.
#define max_conn 64
#define prepare_threads 8192 
// 1/4 l-> join.
#define thresh_gap 1/8.0f
// pixel range.
...
__kernel void quadrilateral_prepare(global char* conn, global float* lines, global int* len, global int* reduced_conns) {
	...
	for (int i = p0; i < np; i += prepare_threads) {
		...
		//int first = get_global_id(0), second = get_global_id(1);
		//int nlines = len[0];
		//if (first > second || first >= nlines || second >= nlines) return;
...
#define reduce_sz_dc 32


//todo: fix this for nv. 
__kernel void rect_decoding(global const unsigned char* rectified, global int* len, int iw, global int* result, global int* ordering, global int* cm) {
	int id = get_group_id(0);
	if (id >= len[1]) return;
	int lid = get_local_id(0);
	local int cval[64]; 
	...
	for (int i = 0; i < 576 / reduce_sz; ++i) { 
		int me = lid + i * reduce_sz; 
		int ix = me % 24, iy = me / 24;
		int x = ix % 3 + (ix / 3) * 5 + 1;
		int y = iy % 3 + (iy / 3) * 5 + 1;
		atomic_add(&cval[(iy / 3) * 8 + ix / 3], rectified[id * rec_sz + y * 40 + x]);
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	for (int i = 0; i < 36 / reduce_sz + (36 % reduce_sz > 0); ++i) {
		...
		if (me < 36) {
			...
			int my = me / 6 + 1;
			int mid = my * 8 + mx;
			int a[9] = { cval[mid - 9], cval[mid - 8],cval[mid - 7],
				        cval[mid - 1], cval[mid], cval[mid + 1],  
				        cval[mid + 7],cval[mid + 8],cval[mid + 9] };
			SWAP(a, 0, 1);
			SWAP(a, 3, 4);
			SWAP(a, 6, 7);
			SWAP(a, 1, 2);
			...
			int lv = a[1] - a[0], now, spliter = a[1]+a[0];
			now = a[2] - a[1]; if (now > lv) { lv = now; spliter = a[2] + a[1]; };
			now = a[3] - a[2]; if (now > lv) { lv = now; spliter = a[3] + a[2]; };
			now = a[4] - a[3]; if (now > lv) { lv = now; spliter = a[4] + a[3]; };
			now = a[5] - a[4]; if (now > lv) { lv = now; spliter = a[5] + a[4]; };
			now = a[6] - a[5]; if (now > lv) { lv = now; spliter = a[6] + a[5]; };
			now = a[7] - a[6]; if (now > lv) { lv = now; spliter = a[7] + a[6]; };
			now = a[8] - a[7]; if (now > lv) { lv = now; spliter = a[8] + a[7]; };

			if (cval[mid] > spliter / 2)
				c[me] = 0;
			else
				c[me] = 1;

			//if (lv > (a[7] - a[0]) * 0.25f) { // bw distinguishable.
			//	if (cval[mid] > spliter / 2) 
			//		c[me] = 0;
			//	else   
			//		c[me] = 1;
	...
	//todo: memory leak causes ordering changes?????
	for (int i = 0; i < 36 / reduce_sz + (36 % reduce_sz > 0); ++i) {
		int me = lid + i * reduce_sz;
