...
namespace ThreeCs.Renderers
{
    ...
    public class WebGLRenderer : IDisposable
    {
        ...
        public bool _logarithmicDepthBuffer = false;
        ...
        private static bool glExtensionCompressedTextureS3TC;
        ...
        public LightCollection _lights = new LightCollection();
        ...
        ///     Constructor
        /// </summary>
        public WebGLRenderer(Control control) //only one renderer.
        {
            ...
            this.shadowMapPlugin = new ShadowMapPlugin(this);
        ...
        public void SetRenderTarget(WebGLRenderTarget renderTarget)
        {
            ...
            if ((null != renderTarget) && (renderTarget.__webglFramebuffer <= 0))
            {
                ...
                var glFormat = (OpenTK.Graphics.OpenGL.PixelFormat) paramThreeToGL(renderTarget.Format);//.FormatOpenTK.Graphics.OpenGL.PixelFormat.Bgra;
...
*/
        ...
        {
            foreach (var object3D in scene._objectsAdded)
                AddObject(object3D, scene);
            scene._objectsAdded.Clear();

            foreach (var object3D in scene._objectsRemoved)
                AddObject(object3D, scene);
            scene._objectsRemoved.Clear();
        }

        void projectObject(Scene scene, Object3D obj, Camera camera)
        {

            if (obj.Visible == false) return;

            var webglObjects = scene._webglObjects.ContainsKey(obj.id) ? scene._webglObjects[obj.id] : null;

            if (webglObjects!=null && (obj.FrustumCulled == false || _frustum.intersectsObject(obj) == true))
            {
                UpdateObject(scene, obj);
                ...
                for (int i = 0, l = webglObjects.Count; i < l; i++){
                    ...
                    if (SortObjects == true)
                    {
                        if (obj.RenderDepth != -1)
                        {
                            webglObject.z = obj.RenderDepth;
                        }
                        else
                        {
                            webglObject.z = new Vector3().SetFromMatrixPosition(obj.MatrixWorld).ApplyProjection(_projScreenMatrix).Z;
                        }
            ...
            {
                projectObject(scene, obj.Children[i], camera);
            }
        ...
        public void Render(Scene scene, Camera camera, WebGLRenderTarget renderTarget = null, bool forceClear = false)
        {
            ...
            this._frustum.SetFromMatrix(_projScreenMatrix); // TODO
            ...
            foreach (var webglObject in scene._webglObjectsImmediate)
            {
                ...
                if (object3D.Visible)
                {
                    ...
                    //todo:UnrollImmediateBufferMaterial( webglObject );
                }
            }
            ...
            if (null != scene.OverrideMaterial)
            {
                ...
                this.RenderObjectsImmediate(scene._webglObjectsImmediate, string.Empty, camera, scene._lights, fog, false, material);
            }
            else
            {
                ...
                this.RenderObjectsImmediate(scene._webglObjectsImmediate, "opaque", camera, lights, fog, false, material);
                
                // transparent pass (back-to-front Order)

                this.RenderObjects(this.transparentObjects, camera, lights, fog, true, material);
                this.RenderObjectsImmediate(scene._webglObjectsImmediate, "transparent", camera, scene._lights, fog, true, material);
            }
        ...
        public void InitGl()
        {
            ...
            {
                glExtensionCompressedTextureS3TC = true;
            }
        ...
        private static void DoClearColor()
        {
        ...
        private void AddBuffer(Dictionary<int, List<WebGlObject>> objlist, BaseGeometry buffer, Object3D object3D)
        {
            ...
            if (!objlist.TryGetValue(id, out var webGlObjects))
            {
     ...
     //     	    delete this._webglObjects[object3D.id].;
            ...
            foreach (var geometryGroup in geometryGroupsList)
            {
                ...
                {
                    this.AddBuffer(scene._webglObjects, geometryGroup, object3D);
                }
            }
        ...
        private void AddObject(Object3D object3D, Scene scene)
        {
            if (object3D.__webglInit == false)
            {
                ...
                {
                    (sender as Object3D)?.Traverse(child => {
                        child.Removed -= OnObjectRemoved;
                        RemoveObject(child, scene);
                    });
                }
                object3D.Removed += OnObjectRemoved;
            }
            ...
            if (object3D.__webglActive == false)
            {
                ...
                if (object3D is Mesh)
                {
                    ...
                    {
                        this.AddBuffer(scene._webglObjects, object3D.Geometry as BufferGeometry, object3D);
                    }
                ...
                {
                    this.AddBuffer(scene._webglObjects, object3D.Geometry, object3D);
                }
                else if (object3D is ImmediateRenderObject && ((ImmediateRenderObject)object3D).immediateRenderCallback != null)
                {
                    this.AddBufferImmediate(scene._webglObjectsImmediate, object3D);
                }
        ...
        /// <param name="object3D"></param>
        public void RenderBuffer(Camera camera, IEnumerable<Light> lights, Fog fog, Material material, BaseGeometry geometryGroup, Object3D object3D)
        {
            ...
            if (updateBuffers)
            {
                ...
                attributeLocation = attributesLocation["custom"];
                if (null != attributeLocation)
                {
                    GL.BindBuffer(BufferTarget.ArrayBuffer, geometryGroup.__webglNormalBuffer);
                    this.EnableAttribute((int)attributeLocation);
            ...
            if (object3D is Mesh)
            {
                ...
                else
                {
                    ...
                    GL.DrawElements(BeginMode.Triangles, geometryGroup.__webglFaceCount, type, 0);
        ...
        /// <param name="geometry"></param>
        /// <param name="object3D"></param>
        public void RenderBufferDirect(Camera camera, IEnumerable<Light> lights, Fog fog, Material material, BufferGeometry geometry, Object3D object3D)
        {
            if (material.Visible == false)
            {
                return;
        ...
        public void RenderImmediateObject(Camera camera, IEnumerable<Light> lights, Fog fog, Material material, WebGlObject webGlObject)
        {
  	        var object3D = webGlObject.object3D;
            ...
            //     webGlObject.immediateRenderCallback( program, _frustum );
            // } else {
            //     webGlObject.render( function ( object3D ) { RenderBufferImmediate( object3D, program, material ); } );
            // }
        }
        ...
        private static void SetDirectBuffers(BufferGeometry geometry, BufferUsageHint hint)
        {
            ...
            var attributesKeys = geometry.Attributes.Keys;
        ...
        private void InitMaterial(Material material, IEnumerable<Light> lights, Fog fog, Object3D object3D)
        {
            ...
            if (program == null)
            {
                ...
                Console.WriteLine($"New Shader Program {program.Id}, GLP:{program.Program}");
            ...
            foreach (var u in ((WebGlShader)material["__webglShader"]).Uniforms)
            {
                ...
                if (location != null)
                {
                    ...
                    material.UniformsList.Add(new UniformLocation() { Uniform = uniform, Location = (int)location , variable = u.Key});
                }
            }
        }
            ...
            //        Console.WriteLine("loadUniformsGeneric: {0} {1} {2}", location, type, value);
                    ...
                    switch (type)
                    {
                        ...
                        case "2fv":
                            ...
                            GL.Uniform2( location, twofv.Length/2, twofv );
                            break;

                        case "3fv":
                            var threefv = ((List<float>)value).ToArray();
                            GL.Uniform3( location, threefv.Length/3, threefv );
                            break;

                        case "4fv":
                            var fourfv = ((List<float>)value).ToArray();
                            GL.Uniform4( location, fourfv.Length/4, fourfv );
                            break;

                        case "Matrix3fv":
                            var matrix3fv = ((List<float>)value).ToArray();
                            GL.UniformMatrix3(location, 1, false, matrix3fv);
                            break;

                        case "Matrix4fv":
                        {//????
                            var matrix4fv = ((List<float>) value).ToArray();
                            GL.UniformMatrix4(location, 1, false, matrix4fv);
                            break;
                        }
                        case "i":
                            GL.Uniform1(location, (int)value);
                        ...
                        case "v2v":
                        {
                            ...
                            var ls = (List<Size>) value;
                            var arr = new float[ls.Count * 2];
                            for (int i = 0; i < ls.Count; ++i)
                            {
                                arr[i*2] = ls[i].Width;
                                arr[i*2+1] = ls[i].Height;
                            }

                            GL.Uniform2(location, ls.Count, arr);
                            break;
                        }
                        case "v3v":
                        ...
                        case "m4v":
                        {
                            var matrix4 = ((List<Matrix4>) value).ToArray();
                            float[] data = new float[16*matrix4.Length];
                            for (int k = 0; k < matrix4.Length; ++k)
                                matrix4[k].elements.CopyTo(data, k * 16);
                            GL.UniformMatrix4(location, matrix4.Length, false, data);
                            break;
                        }
                        case "t":
                        ...
                        case "tv":
                        {
                            ...
                            var textures = (List<Texture>) value;
                            int[] arr = new int[textures.Count];
                            for (int i = 0; i < textures.Count; ++i)
                                arr[i] = GetTextureUnit();
                            GL.Uniform1(location, textures.Count, arr);

                            for (int i = 0; i < textures.Count; ++i)
                            {
                                // var bmp = ((WebGLRenderTarget) textures[i]).Download();
                                SetTexture(textures[i], arr[i]);
                            }
        ...
        private void RefreshUniformsCommon(Uniforms uniforms, Material material)
        {
            ...
            if (material is IMap)
            {
                ...
                Uniforms.SetValue(uniforms, "map", imap.Map);
                Uniforms.SetValue(uniforms, "lightMap", imap.LightMap);
                Uniforms.SetValue(uniforms, "specularMap", imap.SpecularMap);
                Uniforms.SetValue(uniforms, "alphaMap", imap.AlphaMap);
            ...
            if (material is MeshLambertMaterial)
            {
                ...
                Uniforms.SetValue(uniforms, "useRefract",
                    m.EnvMap != null && m.EnvMap.Mapping is Three.CubeRefractionMapping);
                // others here?
            }
        ...
        private void SetBlending(int blending, int blendEquation=0, int blendSrc=0, int blendDst=0)
        {
            ...
            if (blending == Three.CustomBlending)
            {
                ...
                {
                    GL.BlendEquation((ArbDrawBuffersBlend) paramThreeToGL(blendEquation));
                    this._oldBlendEquation = blendEquation;
                }
                if (blendSrc != this._oldBlendSrc || blendDst != this._oldBlendDst)
                {
                    GL.BlendFunc((BlendingFactor) paramThreeToGL(blendSrc), (BlendingFactor) paramThreeToGL(blendDst));
 ...
 * */
        ...
        { 
            ...
            foreach (var light in lights) 
            {
                ...
                else if ( light is SpotLight ) 
                {
                    ...
                    this._direction = vector3 - new Vector3().SetFromMatrixPosition(spotLight.target.MatrixWorld);
				    this._direction.Normalize();
     ...
     */
            ...
            if ( uniforms["shadowMatrix"] != null)
            {
                ...
                foreach ( var light in lights )
                {
                    if (light is ILightShadow)
                    {
                        ...
                        if (light is SpotLight || (light is DirectionalLight dlight && !dlight.shadowCascade))
                        {
                            if (((List<Texture>) uniforms["shadowMap"]["value"]).Count <= j)
                            {
                            ...
                            {
                                ((List<Texture>)uniforms["shadowMap"]["value"])[j]=(lightShadow.shadowMap);
                                ((List<Size>)uniforms["shadowMapSize"]["value"])[j] = (lightShadow.shadowMapSize);

                                ((List<Matrix4>)uniforms["shadowMatrix"]["value"])[j] = (lightShadow.shadowMatrix);

                                ((List<float>)uniforms["shadowDarkness"]["value"])[j] = (lightShadow.shadowDarkness);
                                ((List<float>)uniforms["shadowBias"]["value"])[j] = (lightShadow.shadowBias);
                            }
        ...
        {
            uniforms["ambientLightColor"]["value"] = lights.ambient.colors;
        ...
        /// <param name="texture"></param>
        /// <param name="slot"></param>
        private void SetTexture(ITexture itexture, int slot)
        {
            var texture = (Texture) itexture;
            ...
            if (texture.NeedsUpdate)
            {
                ...
                // GL.PixelStore( PixelStoreParameter.u     GL.UNPACK_FLIP_Y_WEBGL, texture.flipY );
                ...
                var glFormat = (OpenTK.Graphics.OpenGL.PixelFormat)paramThreeToGL(texture.Format);
                var glInternalFormat = (PixelInternalFormat)paramThreeToGL(texture.Format);
                var glType = (PixelType)paramThreeToGL(texture.Type);
                ...
                if (texture is DataTexture)
                {
                    ...
                    if (mipmaps.Count > 0 && isImagePowerOfTwo)
                    {
                        for (var i = 0; i < mipmaps.Count; i++)
                        {
                            ...
                            GL.TexImage2D(TextureTarget.Texture2D, i, glInternalFormat, mipmap.Width, mipmap.Height, 0, glFormat, glType, mipmap.Data);
                        }
                    ...
                    else
                    {
                        ...
                        GL.TexImage2D(TextureTarget.Texture2D, 0, glInternalFormat, image.Width, image.Height, 0, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, glType, data.Scan0);
                ...
                else
                {
                    ...
                    if (null != mipmaps && mipmaps.Count > 0 && isImagePowerOfTwo)
                    {
                        ...
                        {
                            var mipmap = mipmaps[i];
                            GL.TexImage2D(TextureTarget.Texture2D, i, glInternalFormat, image.Width, image.Height, 0, glFormat, glType, mipmap.Data);
                        }
                        texture.GenerateMipmaps = false;
                    }
                    else
        ...
        private void SetTextureParameters(TextureTarget textureTarget, ITexture texture, bool isImagePowerOfTwo)
        {
            ...
            {
                GL.TexParameter(textureTarget, TextureParameterName.TextureWrapS, paramThreeToGL(texture.WrapS));
                GL.TexParameter(textureTarget, TextureParameterName.TextureWrapT, paramThreeToGL(texture.WrapT));

                GL.TexParameter(textureTarget, TextureParameterName.TextureMagFilter, paramThreeToGL(texture.MagFilter));
                GL.TexParameter(textureTarget, TextureParameterName.TextureMinFilter, paramThreeToGL(texture.MinFilter));
            }
            else
            {
...
/*
        ...
        /// <param name="p"></param>
        /// <returns></returns>
	    public static int paramThreeToGL (int p ) {
