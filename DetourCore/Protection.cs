...
namespace DetourCore
{
    class Protection
    {
        ...
        public Protection(string hexKey)
        {
        ...
        public Protection(byte[] cipherKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
        }
        ...
        public static void Validate(string key)
        {
            ...
            bool tryStandalone()
            {
                try
                {
                    G.buyer = Encoding.UTF8.GetString(
                        Convert.FromBase64String(
                            new Protection($"Detour-{G.GetID()}").Decrypt_CBC(key)));
                    G.licenseType = LicenseType.StandAlone;
                    return true;
                }
                catch
                {
            ...
            bool tryModel()
            {
                try
                {
                    G.buyer = Encoding.UTF8.GetString(
                        Convert.FromBase64String(
                            new Protection($"Detour-Model").Decrypt_CBC(key)));
                    G.licenseType = LicenseType.Model;
                    return true;
                }
                catch
                {
            ...
            bool tryEnterprise()
            {
                try
                {
                    G.buyer = Encoding.UTF8.GetString(
                        Convert.FromBase64String(
                            new Protection($"Detour-Enterprise").Decrypt_CBC(key)));
                    G.licenseType = LicenseType.Enterprise;
                    return true;
                }
                catch
                {
            ...
            G.buyer = "试用版";
            G.licenseType = LicenseType.Test;
