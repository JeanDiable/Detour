...
__kernel void calc_line_2(global unsigned int* stat, global int* len, global float* lines) {
	...
	if (l2 / l1 <= 0.333) { // should be 0
		lines[p0 * 12 + 7] = (maxx - minx)*(maxx - minx) + (maxy - miny)*(maxy - miny); //length, should be >16px
	}
	else
...
__kernel void reduce_line(global int* len, global float* lines, global float* rlines) {
	...
	for (int i = 0; i < len[0] / reduce_sz + (len[0] % reduce_sz > 0); ++i) {
		...
		bool is_point = x0 < len[0] && lines[x0 * 12 + 7] >= 16;
...
__kernel void cropping(global const unsigned char* pix, global unsigned char* gpu_cropped, global float2* mesh, int iw) {
	const int x = get_global_id(0), y = get_global_id(1);
	const float mesh_sz = ((float)algo_sz / (croping_mesh - 1));
	const int mesh_x = floor(x / mesh_sz);
	const int mesh_y = floor(y / mesh_sz);


	float ax = x / mesh_sz - mesh_x;
	float ay = y / mesh_sz - mesh_y;
	float bx = 1 - ax;
	float by = 1 - ay;
	...
	gpu_cropped[p0] = //(int)(mesh[mesh_x + mesh_y*croping_mesh].x);// pix[mx + iw*my];
		pix[mx + my*iw] * bx*by +
		pix[mx + (my + 1)*iw] * bx*ay +
		pix[mx + 1 + my*iw] * ax*by +
		pix[mx + 1 + (my + 1)*iw] * ax*ay;
}
