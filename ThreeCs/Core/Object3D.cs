namespace ThreeCs.Core
{
    ...
    public class Object3D : Hashtable, ICloneable, IDisposable
    {
        ...
        public event EventHandler<Scene> AddedToScene;

        public virtual void InvokeAddedToScene(Scene scene)
        {
            var handler = this.AddedToScene;
            if (handler != null)
            {
                handler(this, scene);
            }
        ...
        public event EventHandler<Scene> RemovedFromScene;

        public virtual void InvokeRemovedFromScene(Scene scene)
        {
            var handler = this.AddedToScene;
            if (handler != null)
            {
        ...
        public void Add(Object3D object3D)
        {
		    ...
		    if ( object3D is Object3D )
            {
                ...
                while (scene.Parent != null)
                    scene = scene.Parent;
                if (scene != null && scene is Scene s)
                    s.__addObject(object3D);
            } 
            else
            {
